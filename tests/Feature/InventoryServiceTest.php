...
it('subtracts stock on salida', function () {
    ...
    $this->service->recordMovement(
        ...
        StockMovementType::Entrada, StockMovementReason::Compra, 20,
    );

    // Then remove some
    $this->service->recordMovement(
        $this->variant, $this->location,
        StockMovementType::Salida, StockMovementReason::Merma, 5,
    );
...
it('handles positive and negative adjustments', function () {
    $this->service->recordMovement(
        $this->variant, $this->location,
        StockMovementType::Entrada, StockMovementReason::Compra, 10,
    );

    // Positive adjustment (counted more than system)
    $this->service->recordMovement(
        $this->variant, $this->location,
        StockMovementType::Ajuste, StockMovementReason::AjusteConteo, 3,
    );
    ...
    $this->service->recordMovement(
        $this->variant, $this->location,
        StockMovementType::Ajuste, StockMovementReason::AjusteConteo, -2,
    );
...
it('creates inventory level automatically if it does not exist', function () {
    ...
    $this->service->recordMovement(
        ...
        StockMovementType::Entrada, StockMovementReason::Compra, 5,
    );
...
it('transfers stock between locations', function () {
    ...
    expect($outMovement->type)->toBe(StockMovementType::Salida);
    expect($outMovement->reason)->toBe(StockMovementReason::TransferenciaSalida);
    expect($inMovement->type)->toBe(StockMovementType::Entrada);
    expect($inMovement->reason)->toBe(StockMovementReason::TransferenciaEntrada);
...
it('records all movements immutably', function () {
    ...
    $this->service->recordMovement(
        $this->variant, $this->location,
        StockMovementType::Salida, StockMovementReason::Merma, 3,
    );

    expect(StockMovement::count())->toBe(2);
    expect(StockMovement::where('type', 'entrada')->first()->quantity)->toBe(10);
    expect(StockMovement::where('type', 'salida')->first()->quantity)->toBe(3);
});
